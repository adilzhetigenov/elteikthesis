...
\small
\begin{tabular}{ccc}
& \fbox{\texttt{Browser (Vue.js)}} & \\
& $\updownarrow$ HTTP/Thrift & \\
& \fbox{\texttt{CodeChecker Server (Python)}} & \\
& $\updownarrow$ SQLAlchemy & \\
& \fbox{\texttt{Database (SQLite/PostgreSQL)}} & \\
\\
\fbox{\texttt{report-converter}} & $\rightarrow$ \texttt{coverage.json} $\rightarrow$ & \fbox{\texttt{CodeChecker store}} \\
$\uparrow$ & & $\downarrow$ \\
\texttt{coverage.info} & & Database \\
\end{tabular}
\caption{System architecture showing the connections between components. The browser communicates with the server via Thrift RPC over HTTP. The server accesses the database via SQLAlchemy ORM. The \texttt{report-converter} and \texttt{CodeChecker store} are CLI tools that write to the database independently of the server.}
\label{fig:sysarch}
\end{figure}
...
If a source file referenced in \texttt{coverage.json} is not found in the database through any of the six matching strategies, the storage method attempts to store it directly: it reads the file from disk, computes its SHA-256 hash, creates a \texttt{FileContent} record (if the hash is new) and a \texttt{File} record, then proceeds with coverage storage. This ensures that coverage data can be stored even without a prior static analysis run, as long as the source files are accessible on the machine running \texttt{CodeChecker store}.
...
The line count and function statistics are encoded into the \texttt{SourceFileData} response object's optional fields (\texttt{fileContent} for line counts as \texttt{"hit/total"}, \texttt{remoteUrl} for function stats as \texttt{"found/hit"}).

Encoding data into existing fields is a pragmatic trade-off. The Thrift IDL (\texttt{report\_server.thrift}) was modified to add the new API methods (\texttt{getTestCoverage}, \texttt{getFilesWithCoverage}), and the Python server stubs were regenerated accordingly. However, defining a new return type (e.g., \texttt{CoverageFileData} with explicit \texttt{lineHit}, \texttt{lineTotal}, \texttt{functionsFound}, \texttt{functionsHit} fields) would also require regenerating the JavaScript client library used by the Vue.js frontend. By reusing the existing \texttt{SourceFileData} struct and encoding the statistics into its optional string fields, the frontend can parse the data without regenerating the JavaScript Thrift client. This is a temporary solution---once the feature is stable, a dedicated Thrift struct should replace the field encoding.
...
After extensive investigation, this approach proved fundamentally incompatible with CodeChecker's architecture. The core issue is that CodeChecker's workflow is built around the \emph{compilation database} (\texttt{compile\_commands.json}), which records how each source file is compiled but contains no information about how to \emph{execute} the resulting software. The fundamental limitation is best illustrated by the following observation:
...
The trade-off is storage volume: the xerces-c project produces 66,649 rows in \texttt{test\_coverage} plus 575 rows in \texttt{test\_coverage\_summary}. In practice, this is negligible for SQLite or PostgreSQL, and the indexed \texttt{file\_id} column keeps query times under 100ms even for the full dataset. A natural future extension would be to add an \texttt{execution\_count} column to \texttt{test\_coverage}, storing the actual number of times each line was executed rather than just the covered/uncovered distinction.
...
\hline
LCOV conversion & $<$ 1 s & 93K-line \texttt{.info} $\rightarrow$ 414 KB JSON \\
\hline
\texttt{CodeChecker store} & $\sim$ 11 s & 580 files, 66,649 DB rows \\
\hline
\texttt{getFilesWithCoverage} & $<$ 0.1 s & Single SQL query, 580 files \\
\hline
Statistics page load & $<$ 1 s & Single API call, client-side tree \\
\hline
...
\begin{enumerate}
    ...
    \item Captured coverage with \texttt{lcov} (335 \texttt{.gcda} files, 712 source files in LCOV output)
    \item Converted with \texttt{report-converter -t lcov} (580 files with coverage data, 40,210 covered lines, 8,981 functions found, 5,952 functions hit). The difference between 712 LCOV source files and 580 stored files is due to system headers and files without any executable lines being filtered out during the store operation.
...
\begin{description}
    \item[\textbf{isExecutableLine heuristic was wrong}] The original frontend used a JavaScript function called \texttt{isExecutableLine()} that attempted to determine which source lines are executable by pattern-matching the line content. The heuristic classified lines as non-executable if they matched patterns such as: empty lines, lines starting with \texttt{//} or \texttt{/*} (comments), lines starting with \texttt{\#} (preprocessor directives), lines containing only \texttt{\}} (closing braces), and lines matching \texttt{return <identifier>;} (simple return statements).

    This heuristic produced two types of errors. \emph{False positives}: lines like \texttt{using namespace X;}, \texttt{class Foo \{}, \texttt{public:}, and standalone opening braces \texttt{\{} were classified as executable, but the compiler does not instrument them (no \texttt{DA:} record in LCOV). \emph{False negatives}: lines like \texttt{return fUnicodeForm;} and closing braces of destructors were classified as non-executable by the heuristic's patterns, but the compiler \emph{does} instrument them. Comparing the heuristic against actual LCOV \texttt{DA:} records for \texttt{CreateDOMDocument.cpp} revealed 20 mismatches out of 83 lines the heuristic considered executable (vs.\ 63 actual \texttt{DA:} records). The fix was to store all \texttt{DA:} records (both covered and uncovered) and use them directly for line coloring, eliminating the heuristic entirely.
