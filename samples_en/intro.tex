...
\textbf{SonarQube} integrates coverage visualization with static analysis in a single platform, similar to our goal. However, SonarQube uses its own analysis engine and is a separate product from the compiler toolchain. CodeChecker, by contrast, is built on LLVM/Clang and integrates directly with the compiler's static analyzers. Since both GCC and Clang support the \texttt{--coverage} flag for coverage instrumentation, the addition of compiler-generated coverage data is a natural extension of CodeChecker's existing toolchain.

\textbf{gcov}~\cite{gcov} is the GCC coverage tool that produces per-file \texttt{.gcov} annotated source listings. A \texttt{.gcov} file is a text format where each line of the original source is prefixed with its execution count (or \texttt{\#\#\#\#\#} for unexecuted lines). Our LCOV converter accepts the more widely used LCOV \texttt{.info} format, which aggregates data from multiple source files and is the standard output of CI coverage pipelines.
